# Generated by Powerlevel10k configuration wizard on 2023-08-31 at 07:22 CEST.
...
# 2 lines, solid, no frame, blue-ornaments, compact, few icons, concise,
...
#    newline                 # \n
  ...
  ##############[ taskwarrior: taskwarrior task count (https://taskwarrior.org/) ]##############
  # Taskwarrior color.
  typeset -g POWERLEVEL9K_TASKWARRIOR_FOREGROUND=#00FFFF
  ...
  function prompt_hostname() {
    p10k segment -f 2 -i '' -t "$USER@$(hostname)'"
  }
  function prompt_myalert() {
    [[ $(source $HOME/bin/alert_log.sh) = ":" ]] && MYCOL=black || MYCOL=red
    RESULT=$(source $HOME/bin/alert_log.sh)
    [[ $RESULT = *"✔️"* ]] && MYCOL="#39FF14" 
    p10k segment -f $MYCOL -i '' -t " $RESULT"
  }
  function prompt_myalert2() {
    [[ $(source $HOME/bin/alert_log.sh) = ":" ]] && MYCOL=black || MYCOL=red
    RESULT=$(source $HOME/bin/alert_log.sh)
    [[ $RESULT = *"✔️"* ]] && MYCOL="#39FF14" 
    p10k segment -f $MYCOL -i '' -t " $(cat ~/www/alert.txt)"
  }
  function prompt_myubuntu() {
    DIST=$(cat /etc/os-release | grep NAME)
    REL=$(cat /etc/os-release | grep VERSION_ID | cut -d '"' -f 2)
    [[ $DIST = *"Ubuntu"* ]] && MYUBU="󰬂 $REL" || MYUBU="not Ubuntu"
    p10k segment -f blue -i '' -t "$(echo $MYUBU)"
  }
  function prompt_bashhub() {
    COUNT=$(timeout 3s bh | wc -l)
    [[ $COUNT -gt "66" ]] && MYBH="󰯮 󰰀 ok" || MYBH="󰫯󰫵"
    [[ $COUNT -gt "66" ]] && MYC="green" || MYC="red"
    p10k segment -f $MYC -i '' -t "$(echo $MYBH)"
  }
  function prompt_twingate() {
    STAT=$(timeout 3s twingate status)
    [[ $STAT = "online" ]] && MYTG="󰰤 󰯽 ok" || MYTG="󰰤 󰯽󰫯"
    [[ $STAT = "online" ]] && MYC2="green" || MYC2="red" && {twingate status | apprise ntfys://n.yyps.de/alert}
    [[ $STAT != "online" ]] && twingate status | curl -O -s https://n.yyps.de/alert 
    p10k segment -f $MYC2 -i '' -t "$(echo $MYTG)"
  }
  function prompt_myalert2() {
#    p10k segment -f red -i '' -t "$(source $HOME/bin/alert_log2.sh)"
    p10k segment -f red -i '' -t "$(cat ~/www/logs/alert_log)"

  }
  function prompt_screenshot() {
    SCREENSHOT=$(cat ~/www/logs/screenshot)
    MYCOLOR="$(echo $SCREENSHOT | sed 's/----.*//')"
    #echo $MYCOLOR
    SCREENSHOT=$(echo $SCREENSHOT | sed 's/.*----//')
    #[[ $(echo $SCREENSHOT | sed 's/h .*//' | sed 's/.* //') -lt "1" ]] && MYCOLOR="blue" || MYCOLOR="green"
    p10k segment -f $MYCOLOR -i '' -t "$SCREENSHOT "
 #   p10k segment -f $MYCOLOR -i '' -t "$SCREENSHOT "
  }
